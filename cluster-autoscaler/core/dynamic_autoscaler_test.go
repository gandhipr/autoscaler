...
func (m *AutoscalerMock) Start() error {
	m.Called()
	return nil
}
...
func TestRunOnce(t *testing.T) {
	t.Run("Test RunOnce with no config updates", func(t *testing.T) {
	...
	t.Run("Test RunOnce with config updates", func(t *testing.T) {
		currentTime := time.Now()

		newConfig := dynamic.NewDefaultConfig()

		initialAutoscaler := &AutoscalerMock{}
		// ensure we cleanup the old autoscaler
		initialAutoscaler.On("ExitCleanUp").Once()

		newAutoscaler := &AutoscalerMock{}
		newAutoscaler.On("RunOnce", currentTime).Once()
		newAutoscaler.On("Start").Once()
