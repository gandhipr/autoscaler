...
func TestInitializeCloudProviderRateLimitConfigWithInvalidReadAndWriteRateLimitSettingsFromEnv(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedErr:                     true,
			expectedErrMsg: fmt.Errorf("failed to parse %s: %q, strconv.ParseFloat:"+
				" parsing \"invalid\": invalid syntax", rateLimitReadQPSEnvVar, invalidSetting),
		},
		{
			desc:                                "an error shall be returned if invalid rateLimitReadBucketsEnvVar",
			isInvalidRateLimitReadBucketsEnvVar: true,
			expectedErr:                         true,
			expectedErrMsg: fmt.Errorf("failed to parse %s: %q, strconv.ParseInt:"+
				" parsing \"invalid\": invalid syntax", rateLimitReadBucketsEnvVar, invalidSetting),
		},
		{
			...
			expectedErr:                      true,
			expectedErrMsg: fmt.Errorf("failed to parse %s: %q, strconv.ParseFloat:"+
				" parsing \"invalid\": invalid syntax", rateLimitWriteQPSEnvVar, invalidSetting),
		},
		{
			...
			expectedErr:                          true,
			expectedErrMsg: fmt.Errorf("failed to parse %s: %q, strconv.ParseInt:"+
				" parsing \"invalid\": invalid syntax", rateLimitWriteBucketsEnvVar, invalidSetting),
		},
	}
